...
public class HHCFMainReal<S extends Solution<?>> {
  ...
  public HHCFMainReal(Problem<IntegerSolution> problem, String[] argspar) throws FileNotFoundException, ConfigurationException, JMException, IOException {
    ...
    public void mainHHCFReal(Problem<IntegerSolution> problem, String[] args) throws ConfigurationException, JMException, FileNotFoundException, IOException {
...
//        Problem problem = ProblemFactory.getProblem(problemString, k, l, m); // Create problem instance
        ...
        for (int i = 0; i < trials; i++) { // ******* BEGIN -> TRIALS *******
		  ...
		  allAEs = qualIndMatrices.getAllIndicatorsCF(qualityIndicators, "EPSILON");
		  
		  List<Double> allRNIs = new ArrayList<Double>();
		  allRNIs = qualIndMatrices.getAllIndicatorsCF(qualityIndicators, "IGDPLUS");
...
//				 uniDistValues[qind][1] = allUDs.get(qind);
		  ...
		  int outcomeLLHCF = -1;
  	      ...
  	      outcomeLLHCF = chFun.getMaxChoiceFunction(algEffortValues, 0, rniValues, 0, hypervolumeValues, 1, numberOfMeasures, alpha, elapsedTime);
          ...
          for (int j = 3; j < decisionPoints; j++) { // ******* BEGIN -> DECISION POINTS ******* -> j = 3 -> minus 3 iterations due to the initialisation 
			  ...
			  qualIndMatrices.clearAllIndicatorsCF();
			  outcomeLLHCF = chFun.getMaxChoiceFunction(algEffortValues, 0, rniValues, 0, hypervolumeValues, 1, 
						                                   numberOfMeasures, alpha, elapsedTime);
    ...
    public static List<Double> printResultsAsList(List archive, Problem problem, boolean recordStatEval, int popSize,
        ...
        if (recordStatEval) {
           ...
           stindicators.recordDataStatEval("IGDPLUS", igdValue);
